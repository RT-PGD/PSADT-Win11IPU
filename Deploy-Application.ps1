...
Try {
    ...
    [String]$appScriptDate = '07/03/2024'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        # Copy windows.old cleanup script to local disk
        Copy-File -Path "$dirFiles\*" -Destination "C:\temp\Delete-Windows-dot-old.ps1" -Recurse

        # Create Windows.old Clean up task.
        $ST2Executable = "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"
        $ST2Arguments = "-ExecutionPolicy Bypass -File C:\temp\Delete-Windows-dot-old.ps1"
        $ST2TaskName = "DeleteWindows.old"
        $ST2TriggerTime = (Get-Date).AddDays(1)
        $ST2Priority = 4

        Write-Log -Message "Creating scheduled task $ST2TaskName to run `"$ST2Executable $ST2Arguments`""

        $ST2Trigger = New-ScheduledTaskTrigger -Once -At $ST2TriggerTime
        $ST2Settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -Priority $ST2Priority -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries

        # Create a new scheduled task action.
        $ST2Action = New-ScheduledTaskAction -Execute $ST2Executable -Argument $ST2Arguments -WorkingDirectory (Split-Path $ST2Executable)

        # Create a new scheduled task principal for the SYSTEM account
        $ST2Principal = New-ScheduledTaskPrincipal -UserID "NT AUTHORITY\SYSTEM" -LogonType ServiceAccount -RunLevel Highest

        # Create and register the new scheduled task
        $ST2Task = New-ScheduledTask -Action $ST2Action -Trigger $ST2Trigger -Settings $ST2Settings -Principal $ST2Principal
        Register-ScheduledTask -TaskName $ST2TaskName -InputObject $ST2Task

        #########################################
                #### Check Windows Compability
                #### Download Drivers & ESD
        #########################################
        ...
        #============================================================================
        Write-Log "========================================================================="
        Write-Log "$((Get-Date).ToString('yyyy-MM-dd-HHmmss')) Starting OSDCloudIPU"
...
#>
       ...
       $DELTaskName = "OSDWin11Upgrade"
        ...
        New-Item -ItemType "file" -Path "c:\logfiles\gWin11-23H2-Upgrade.txt"

        # Not Working at this point in upgrade I think the reg has been copied hence why it fails
